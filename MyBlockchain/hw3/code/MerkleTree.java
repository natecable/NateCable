...
public class MerkleTree {
    ...
    public String toString(Leaf root){
        String out = "";
        out = getAllLeaf(root, out);
        return out;
    }

    private String getAllLeaf(Leaf root, String out){
        if(root == null){
            return out;
        }

        if(root.left == null && root.right == null){
            out = out + root.toString();
        }

        if(root.left != null){
            out = getAllLeaf(root.left, out);
        }

        if(root.right != null){
            out = getAllLeaf(root.right, out);
        }
